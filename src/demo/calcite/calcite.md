...
以 [CalciteTest](./CalciteTest.java)._parse()_ 为例
...
与我们的调用代码 [validate](./CalciteTest.java)._validate()_ 看一次验证的过程
...
关于优化我们直接查看代码: [**CalciteTest**](./CalciteTest.java)._cBoRelNodeFindBestExp()_.
...
结合 [**CalciteTest**](./CalciteTest.java)._sQLNode2RelNode()_ 我们来看 SqlNode -> RelNode 的过程.
...
以 [CalciteTest](./CalciteTest.java)._rBoRelNodeFindBestExp()_ 入口, 追踪源码看下:
...
public class HepPlanner extends AbstractRelOptPlanner {
    ...
    private void executeProgram(HepProgram program) {
        ...
        // 遍历的就是: 我们创建优化器时指定的规则
        // 参考创建优化器代码: demo.calcite.CalciteTest.createHepPlanner
